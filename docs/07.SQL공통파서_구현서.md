...
3. **3단계**: 조인관계 추출 및 relationships 테이블 저장
   - JOIN_EXPLICIT/EXPLICIT_OUTER/EXPLICIT_FULL_OUTER (JOIN ... ON)
   - JOIN_IMPLICIT/IMPLICIT_OUTER (WHERE ... 기반)
...
① JOIN ... ON <조인조건> --> JOIN_EXPLICIT
   - LEFT/RIGHT OUTER JOIN --> JOIN_EXPLICIT_OUTER
   - FULL OUTER JOIN --> JOIN_EXPLICIT_FULL_OUTER
② WHERE ... T1.F1 = T2.F2 --> JOIN_IMPLICIT
③ WHERE ... T1.F1 = T2.F2(+) --> JOIN_IMPLICIT_OUTER
④ MERGE ... USING ... ON (<조인조건>) --> JOIN_MERGEON
```

> **분석 제외 범위**  
> - CROSS/NATURAL/USING/비동등(>, <, BETWEEN) 조인은 분석 대상에서 제외합니다.

> **WHERE 범위 한정 규칙 (2025-11-23 반영)**  
> - `JOIN_IMPLICIT`/`JOIN_IMPLICIT_OUTER` 분석 시 WHERE 절은 다음 토큰 전까지만으로 한정합니다.  
...
4. **조인 분석**: JOIN_EXPLICIT/EXPLICIT_OUTER/EXPLICIT_FULL_OUTER/IMPLICIT/IMPLICIT_OUTER/MERGEON 관계 분석 및 관계 저장 (스키마에 없으면 INFERRED 테이블/컬럼 생성으로 관계 보존)
...
flowchart TD
    ...
    E1[JOIN_EXPLICIT 계열 분석] --> E
    E2[JOIN_IMPLICIT 계열 분석] --> E
    E3[관계 통합 및 중복 제거] --> E
```
...
```sql
JOIN ... ON <조인조건>             -- JOIN_EXPLICIT
LEFT/RIGHT OUTER JOIN ... ON ...   -- JOIN_EXPLICIT_OUTER
FULL OUTER JOIN ... ON ...         -- JOIN_EXPLICIT_FULL_OUTER
WHERE ... T1.F1 = T2.F2            -- JOIN_IMPLICIT
WHERE ... T1.F1 = T2.F2(+)         -- JOIN_IMPLICIT_OUTER
MERGE ... USING ... ON (...)       -- JOIN_MERGEON
```
...
def analyze_join_relationships(self, sql_content: str, file_path: str = "", component_id: int = 0):
    ...
    # 3. JOIN_EXPLICIT 계열 분석 (INNER/LEFT/RIGHT/FULL)
    explicit_relationships = self._analyze_explicit_join_chain(...)
    
    # 4. JOIN_IMPLICIT 계열 분석 (WHERE/Oracle (+))
    implicit_relationships = self._analyze_implicit_joins_in_where(...)
    
    # 5. JOIN_MERGEON 분석
    merge_relationships = self._analyze_merge_joins(...)
...
| 기능 | 지원 상태 | 설명 |
|------|-----------|------|
| **JOIN_EXPLICIT** | ✅ 완전 지원 | INNER JOIN |
| **JOIN_EXPLICIT_OUTER** | ✅ 완전 지원 | LEFT/RIGHT OUTER JOIN |
| **JOIN_EXPLICIT_FULL_OUTER** | ✅ 완전 지원 | FULL OUTER JOIN |
| **JOIN_IMPLICIT** | ✅ Oracle 지원 | WHERE 조건의 테이블 연결 분석 |
| **JOIN_IMPLICIT_OUTER** | ✅ 지원 | Oracle 외부 조인 구문 (+) 해석 |
| **JOIN_MERGEON** | ✅ 지원 | MERGE ... USING ... ON |
