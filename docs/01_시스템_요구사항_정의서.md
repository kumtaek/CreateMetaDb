...
- **선택 파라미터**: `--clear-db` (메타데이터베이스 초기화), `--force` (강제 실행), `--verbose` (상세 로그)
- **에러 처리**: 파라미터 미지정 시 명확한 에러 메시지 출력
- **실행 방식**: `python main.py --project-name SampleSrc --force`
...
5. **분석 실행**: `python main.py --project-name {project_name} --force`
