...
```mermaid
flowchart TD
    A["SourceAnalyzer Root"] --> B["메인 실행 파일들"]
    B --> B1["main.py<br/>메인 분석 실행"]
    B --> B2["create_report.py<br/>리포트 생성 실행"]
    B --> B3["file_loading.py<br/>1-2단계 처리"]
    B --> B4["xml_loading.py<br/>3단계 XML 분석"]
    B --> B5["java_loading.py<br/>4단계 Java 분석"]
    B --> B6["backend_entry_loading.py<br/>5단계 API 분석"]
    
    A --> C["설정 및 스키마"]
    C --> C1["config/<br/>시스템 설정"]
    C --> C2["database/<br/>DB 스키마"]
    
    A --> D["핵심 모듈들"]
    D --> D1["parser/<br/>파싱 엔진들"]
    D --> D2["reports/<br/>리포트 생성기들"]
    D --> D3["util/<br/>공통 유틸리티들"]
    
    A --> E["프로젝트 데이터"]
    E --> E1["projects/SampleSrc/<br/>분석 대상 프로젝트"]
    E --> E2["metadata.db<br/>메타데이터베이스"]
    E --> E3["report/<br/>생성된 리포트들"]
    E --> E4["logs/<br/>실행 로그들"]
...
3. **3단계 - XML 분석**: Enhanced MyBatis 파싱 (include 태그, 동적 SQL), JOIN 관계 분석, SQL 압축 저장 ✅ **구현 완료**
4. **4단계 - Java 분석**: 클래스/메서드 추출, **Enhanced SQL 추출 (StringBuilder + 정규식)**, 관계 분석, SQL 압축 저장 ✅ **구현 완료**
5. **5단계 - API 매핑**: 백엔드 진입점 분석 및 API 매핑 (Spring Framework 지원) ✅ **구현 완료**
6. **리포트 생성**: 다양한 분석 리포트 생성 (CallChain, ERD, Architecture, Sequence Diagram) ✅ **구현 완료**
7. **7단계 - AI 보완 분석**: AI 기반 고급 분석 및 정확도 향상 🔄 **개발 예정**
...
- **Java**: 클래스, 메서드, 상속 관계, SQL 추출 (StringBuilder 패턴 포함)
- **XML (MyBatis)**: Enhanced 파싱, include 태그, 동적 SQL, JOIN 관계 분석
- **Spring Framework**: 백엔드 진입점 분석 (Controller, RestController)
- **Servlet**: 서블릿 진입점 분석
...
- MyBatis (3.x 동적 SQL + Enhanced 파싱) ✅ **구현 완료**
- Java (클래스, 메서드, 상속 관계 + Enhanced SQL 추출) ✅ **구현 완료**
