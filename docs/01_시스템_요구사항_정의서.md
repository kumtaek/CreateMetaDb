...
| | `reports/` | 리포트 생성기들 | CallChain, ERD, Architecture |
| | `util/` | 공통 유틸리티들 | Database, Hash, Path 등 |
| **📊 프로젝트 데이터** | | | |
...
sequenceDiagram
    ...
    U->>M: python main.py --project-name SampleSrc
    M->>F: 1단계 파일 스캔
    F-->>M: 143개 파일 스캔 완료
    M->>F: 2단계 DB 구조 저장
    F-->>M: 테이블/컬럼 로드 완료
...
sequenceDiagram
    ...
    U->>CR: python create_report.py --project-name SampleSrc
...
8. **리포트 생성**: CallChain, ERD, Architecture, Sequence Diagram ✅ **구현 완료**
...
```
parser/
├── xml_parser.py              # MyBatis XML 분석 (안정적 파싱)
├── java_parser.py             # Java 클래스/메서드 분석
├── frontend_parser.py         # 프론트엔드 통합 파서 (JSP, JSX, Vue, TS, JS, HTML)
...
- SQL 쿼리 추출 (SELECT, INSERT, UPDATE, DELETE)
- JOIN 관계 분석 (EXPLICIT/IMPLICIT)
- 테이블 사용 관계 도출
- SAX Fallback으로 안정성 보장
