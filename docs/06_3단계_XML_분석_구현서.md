...
- **INFERRED 컬럼 등록**: 테이블/컬럼이 조회되지 않으면 INFERRED 컬럼으로 생성해 관계를 보존
...
2.  **공통부 활용**: SqlParser + SqlJoinAnalyzer로 일관된 분석
   ...
   - JOIN_EXPLICIT/EXPLICIT_OUTER/EXPLICIT_FULL_OUTER/IMPLICIT/IMPLICIT_OUTER/MERGEON 관계 분석
   - 테이블/컬럼 자동 추출 및 USE_TABLE/조인 관계 생성
...
- **동작**: MyBatis XML에서 연관관계 구축에 필요한 핵심 정보만 추출
  ...
  - 조인 관계 분석 (JOIN_EXPLICIT/EXPLICIT_OUTER/EXPLICIT_FULL_OUTER/IMPLICIT/IMPLICIT_OUTER/MERGEON)
...
- **동작**: 추출된 SQL에서 테이블 간 JOIN 관계만 단순 분석
  - JOIN_EXPLICIT/EXPLICIT_OUTER/EXPLICIT_FULL_OUTER
  - JOIN_IMPLICIT/IMPLICIT_OUTER
  - JOIN_MERGEON
