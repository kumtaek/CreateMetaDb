...
- **저장**: CLASS, METHOD, SQL_*, TABLE, COLUMN, API_ENTRY, FRONTEND_API, JSP
- **상세 정보**: API_ENTRY, FRONTEND_API는 api_components 테이블에 상세 정보 저장
...
- [⏳] 5단계: API 진입점 분석 (API_ENTRY 컴포넌트 생성 및 프론트엔드 → 백엔드 관계 생성) - **개발 예정**

- [✅] 6단계: JSP 파일 분석 (JSP 컴포넌트 등록 및 JSP → METHOD 관계 생성) - **구현 완료**
...
5. API 진입점 분석 → API_ENTRY 컴포넌트 생성 + 프론트엔드 → 백엔드 관계 생성 (개발 예정)
   - Java 파일에서 백엔드 진입점 식별 (@RestController, @RequestMapping 등)
   - components 테이블에 API_ENTRY, FRONTEND_API 컴포넌트 생성
   - api_components 테이블에 상세 정보 저장 (통합 설계)
   - relationships 테이블에 CALL_API_F2B, CALL_API_BE 관계 생성
   - 완전한 호출 체인 구성: FRONTEND_API → API_ENTRY → METHOD → SQL_* → TABLE
6. JSP 분석 (메모리 최적화) → JSP 컴포넌트 등록 + JSP → METHOD 관계 생성 + JSP → API 관계 분석 ✅ **구현 완료**
   ...
   - JSP에서 API 호출 패턴 분석 및 FRONTEND_API 컴포넌트 생성
   - 완전한 호출 체인: JSP → FRONTEND_API → API_ENTRY → METHOD → SQL_* → TABLE
...
- **5단계 API 진입점 분석**: Java 백엔드 제일 앞단부터 프론트엔드 → 백엔드 호출 체인 구성 (개발 예정)
- **5단계 통합 설계**: API_ENTRY와 FRONTEND_API를 api_components 테이블로 통합 관리
- **5단계 프론트엔드 무관**: React, Vue, Angular 등 프론트엔드 기술에 관계없이 백엔드 진입점 분석
- **5단계 완전한 호출 체인**: FRONTEND_API → API_ENTRY → METHOD → SQL_* → TABLE 구성
- **6단계 JSP 분석**: JSP 파일을 components 테이블에 등록하고 JSP → METHOD 관계 생성 [OK]
- **6단계 API 관계 분석**: JSP에서 API 호출 패턴 분석 및 FRONTEND_API 컴포넌트 생성 [OK]
