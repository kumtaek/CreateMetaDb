> ⚠️ **현행화 노트(2025-11-23)**: INFERRED 테이블/컬럼/파일 자동 생성 로직은 제거되었으며, 모든 컴포넌트·관계 저장 시 실제 `file_id`를 요구합니다. 본문에 남아있는 INFERRED 자동등록 설명은 역사적 기록이며, 최신 정책은 `11_전역컨텍스트_및_파일출처_관리.md`와 `12_INFERRED_제거_정책_및_오류처리.md`를 참고하세요.
