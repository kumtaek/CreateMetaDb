...
> **중요 (OWNER 처리 정책)**  
> - 테이블 매칭은 **테이블명만**으로 수행한다. SQL에 OWNER가 있어도 `TABLE` 이름 기준으로 metadata의 테이블을 찾는다.  
> - 조회 실패 시: SQL에서 추출한 OWNER가 있으면 그 OWNER로 INFERRED 테이블을 생성, 없으면 OWNER=`UNKNOWN`으로 생성한다.  
> - metadata에 OWNER(SAMPLE 등)가 있어도 SQL에 OWNER가 없으면 테이블명으로만 매칭하며, OWNER 표시는 별도 로직을 두지 않는 한 리포트에는 노출되지 않는다.
...
> - “정답지 비교” 등 메타 정확성 검증 단계에서만 일시적으로 원본 소스 재파싱을 허용하며, 이후 일반 실행에서는 메타DB를 신뢰하는 것이 원칙이다.
