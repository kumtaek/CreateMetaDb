merge using (select...)도 그 안에는 FROM ...이 나오잖아. 그래서 서브쿼리던 인라인뷰던 FROM... 을 보고 파싱하면 되.
정확한 파싱이 목표가 아니라고.   초기 컨셉 설계시 개발자와 나눴던 아래 내용 참고해봐.  꼭 아래처럼 하라는 건 아니고
우리 목표와 개발 컨셉을 다시 리마인드하라나는 차원이야.  
어떻게 문제를 개선할지 개선안을 얘기해봐. 아직 소스수정은 하지 말고.
