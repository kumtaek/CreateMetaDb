...
class JavaLoadingEngine:
    ...
    def __init__(self, project_name: str):
        ...
        # 심플 Java 파서 초기화 - public/protected만 추출
        ...
        try:
            ...
            for java_file in java_files:
                try:
                    ...
                    # 컴포넌트 데이터 생성
                    components = self.java_parser.get_component_data(parse_result, file_id, project_id)

                    # 컴포넌트 등록 (UPSERT 방식)
                    for comp in components:
                        self._upsert_component(comp)

                        if comp['component_type'] == 'CLASS':
                            self.stats['classes_extracted'] += 1
                        elif comp['component_type'] == 'METHOD':
                            self.stats['methods_extracted'] += 1

                    debug(f"Java 파일 처리 완료: {java_file} - 클래스: {len(parse_result['classes'])}, 메서드: {len(parse_result['methods'])}")

                    # 3단계 쿼리 분석을 위해 SimpleQueryAnalyzer 호출
                    try:
                        query_analysis = self.simple_query_analyzer.analyze_java_file(java_file, file_id)
                        if query_analysis:
                            self.stats['sql_queries_extracted'] += len(query_analysis.get('queries', []))
                    except Exception as e:
                        warning(f"쿼리 분석 실패: {java_file} - {e}")
                        self.stats['errors'] += 1
            ...
            handle_error(e, "Java 파일 처리 중 시스템 오류")
            return False
    ...
    def _upsert_component(self, comp_data: Dict) -> bool:
        """컴포넌트 UPSERT (있으면 업데이트, 없으면 생성)"""
        try:
            # 기존 컴포넌트 확인
            check_query = """
            ...
            if existing:
                ...
                update_query = """
                    UPDATE components
                    SET line_start = ?, layer = ?, has_error = ?, error_message = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE component_id = ?
                """
                ...
                insert_query = """
                    INSERT INTO components (
                        ...
                        line_start, layer, has_error, error_message, created_at, updated_at, del_yn
...
# 편의 함수
def load_java_files(project_name: str, project_id: int) -> bool:
    """Java 파일 로딩 실행"""
    loader = JavaLoader(project_name)
    ...
    def _is_java_keyword(self, name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Oracle SQL 키워드 확인 (싱글톤 매니저 사용)
                if self.oracle_keyword_manager.is_oracle_keyword(name):
                    return True
            except:
